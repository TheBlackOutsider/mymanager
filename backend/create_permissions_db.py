#!/usr/bin/env python3
"""
Script de création de la structure de base de données pour les permissions
Remplace la définition en code par une approche dynamique
"""

from main import SessionLocal, Base, engine, Employee
...
# Table de liaison many-to-many pour les permissions des utilisateurs
user_permissions = Table(
    'user_permissions',
    Base.metadata,
    Column('user_id', UUID(as_uuid=True), ForeignKey('employees.id')),
    Column('permission_id', UUID(as_uuid=True), ForeignKey('permissions.id'))
)
...
class Permission(Base):
    ...
    users = relationship("Employee", secondary=user_permissions, back_populates="custom_permissions")

# Modèle pour les conditions de permission
class PermissionCondition(Base):
    ...
    field = Column(String, nullable=False)          # user.department, user.seniority
    operator = Column(String, nullable=False)       # eq, in, gt, lt, contains
    value = Column(String, nullable=False)          # Valeur de comparaison
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
def create_permissions_structure():
    ...
    try:
        ...
        print(f"   - Conditions: {db.query(PermissionCondition).count()}")
